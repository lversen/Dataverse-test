...
# Download Dataverse installer
echo "Downloading Dataverse installer..."
cd /home/azureuser
wget -q https://github.com/IQSS/dataverse/releases/download/v6.1/dvinstall.zip
unzip -q dvinstall.zip
chown -R azureuser:azureuser dvinstall

# Create info file
cat > /home/azureuser/dataverse-info.txt <<EOL
Dataverse Prerequisites Installed Successfully!
...
Dataverse installer downloaded to: /home/azureuser/dvinstall

Next steps:
1. cd dvinstall
2. sudo ./install

Access Dataverse at: http://$VMIPAddress:8080

EOL

echo "Setup complete! Check ~/dataverse-info.txt for details."
echo "Ready to install Dataverse!"